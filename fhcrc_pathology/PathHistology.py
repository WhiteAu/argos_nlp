...
def get(disease_group,dictionary,specimen,module_sections):  
        ...
        if re.search(module_sections,header):                    
            for index,results in sorted(dictionary[section].items(),key=lambda x: int(x[0])):               
                ...
                if re.search('[\d]{4}[;,][ ]*[\d]{1,4}:[\d\-]{1,6}',results):pass 
                    ...
                    if histology:                               
                        start_stops_set.add((line_onset+onset,line_onset+offset))                        
                        already_seen=False
    ...
    if specimen_histology_list:
        ...
        confidence=.85
        if len(set(specimen_histology_set))>1:confidence=.7  ## this should lead to "PathFindingLogic" for multiple histologic specimens
        start_stops_list=[]
        ...
        return_dictionary_list.append({global_strings.NAME:"PathHistology",global_strings.VALUE:';'.join(specimen_histology_set),global_strings.CONFIDENCE:("%.2f" % confidence),
                                       global_strings.VERSION:__version__, global_strings.STARTSTOPS:start_stops_list})
